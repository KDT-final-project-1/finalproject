...
public interface IssueStatusMapper {
	...
	// 검색조건에 맞는 사용자별 일감상태 목록을 조회한다.
	List<IssueStatusVO> selectIssueStatusSearchList(IssueStatusVO searchVO);

	// 사용자별 일감 상태 ID 기준으로 상세 정보를 조회한다.
	IssueStatusVO selectIssueStatusDetail(@Param("issueStatusId") String issueStatusId, @Param("userId") String userId);
	...
	// 사용자별 일감 상태를 삭제한다.
	int deleteIssueStatus(@Param("issueStatusId") String issueStatusId, @Param("userId") String userId);

	// 사용자별 상태명 중복 여부를 확인한다.
	int countDuplicateStatusName(@Param("userId") String userId, @Param("statusName") String statusName,
